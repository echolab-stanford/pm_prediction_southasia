...
library(leaflet)
library(mapview)
setwd("C:/Users/ayako/Documents/StanfordPhD/Myprojects/01_PM_prediction/pm_prediction_southasia")
...
grid <- raster(extent(zone), resolution = c(1000,1000), crs = proj)
grid_1km <- rasterToPolygons(grid)
grid_1km_buffer <- gBuffer(grid_1km, byid=T, width=res/2, capStyle="SQUARE")
grid_1km_fin <- st_as_sf(grid_1km_buffer) %>% st_transform(st_crs(4326))
grid_1km_fin <- grid_1km_fin %>% mutate(grid_id = row_number()) %>% 
  select(grid_id, geometry)

st_write(obj=grid_1km_fin, 
                  dsn="C:/Users/ayako/Documents/StanfordPhD/Myprojects/01_PM_prediction/data/intermediate/grid_1km",
